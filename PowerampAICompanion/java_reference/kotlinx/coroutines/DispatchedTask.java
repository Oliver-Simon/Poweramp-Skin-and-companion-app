            ...
            if (oldValue$iv != ThreadContextKt.NO_THREAD_ELEMENTS) {
                try {
                    ...
                    try {
                        ...
                        result = Result.m510constructorimpl(Unit.INSTANCE);
                    } catch (Throwable th2) {
                        th = th2;
                        Result.Companion companion2 = Result.INSTANCE;
                        result = Result.m510constructorimpl(ResultKt.createFailure(th));
                        handleFatalException$kotlinx_coroutines_core(fatalException, Result.m513exceptionOrNullimpl(result));
                    }
                    handleFatalException$kotlinx_coroutines_core(fatalException, Result.m513exceptionOrNullimpl(result));
                }
            } else {
                ...
                try {
                    if (job == null || job.isActive()) {
                        ...
                        if (exception != null) {
                            ...
                            continuation.resumeWith(Result.m510constructorimpl(ResultKt.createFailure(exception)));
                        } else {
                            Result.Companion companion4 = Result.INSTANCE;
                            continuation.resumeWith(Result.m510constructorimpl(getSuccessfulResult$kotlinx_coroutines_core(state)));
                        }
                    } else {
                        ...
                        if (DebugKt.getRECOVER_STACK_TRACES()) {
                            ...
                            try {
                                if (continuation instanceof CoroutineStackFrame) {
                                    ...
                                    continuation.resumeWith(Result.m510constructorimpl(ResultKt.createFailure(cancellationException)));
                                }
                            } catch (Throwable th5) {
                                th = th5;
