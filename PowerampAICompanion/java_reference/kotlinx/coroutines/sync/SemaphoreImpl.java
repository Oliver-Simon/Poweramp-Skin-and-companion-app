...
public class SemaphoreImpl implements Semaphore {
    ...
    */
    ...
    public final boolean addAcquireToQueue(Waiter waiter) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m2072getSegmentimpl(s$iv);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        while (true) {
            findSegmentInternal = ConcurrentLinkedListKt.findSegmentInternal(semaphoreSegment2, j2, semaphoreImpl$tryResumeNextFromQueue$createNewSegment$1);
            if (SegmentOrClosed.m2074isClosedimpl(findSegmentInternal)) {
                j = andIncrement;
                z = false;
                z2 = true;
                break;
            }
            Segment m2072getSegmentimpl = SegmentOrClosed.m2072getSegmentimpl(findSegmentInternal);
            while (true) {
                ...
                j = andIncrement;
                if (segment.id >= m2072getSegmentimpl.id) {
                    z3 = true;
                    break;
                }
                if (!m2072getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z3 = false;
                    break;
                }
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, this, segment, m2072getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                    z3 = true;
                } else {
                    if (m2072getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m2072getSegmentimpl.remove();
                    }
        ...
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m2072getSegmentimpl(findSegmentInternal);
        ...
        Object andSet = semaphoreSegment3.getAcquirers().getAndSet(i4, symbol);
        if (andSet == null) {
            i3 = SemaphoreKt.MAX_SPIN_CYCLES;
            for (?? r11 = z; r11 < i3; r11++) {
                Object obj = semaphoreSegment3.getAcquirers().get(i4);
            ...
            return !Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(semaphoreSegment3.getAcquirers(), i4, symbol3, symbol4);
        }
