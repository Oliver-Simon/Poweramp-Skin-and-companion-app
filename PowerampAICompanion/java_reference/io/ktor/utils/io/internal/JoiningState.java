...
public final class JoiningState {
    ...
    public JoiningState(ByteBufferChannel delegatedTo, boolean delegateClose) {
    ...
    private final Job getCloseWaitJob() {
        CompletableJob newJob;
        do {
            Job current = (Job) this._closeWaitJob;
            if (current != null) {
                return current;
            }
            newJob = JobKt__JobKt.Job$default((Job) null, 1, (Object) null);
        } while (!AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(_closeWaitJob$FU, this, null, newJob));
        if (this.closed == 1) {
            Job.DefaultImpls.cancel$default((Job) newJob, (CancellationException) null, 1, (Object) null);
        }
        return newJob;
    }
