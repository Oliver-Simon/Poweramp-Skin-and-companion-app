...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1552plusAssignLRDsOJo(long duration) {
        long longDelta = Duration.m1479toLongimpl(duration, getUnit());
        ...
        if ($i$f$isSaturated == 0) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m1551overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m1443divUwyO8pc(duration, 2);
        long $this$isSaturated$iv = Duration.m1479toLongimpl(half, getUnit());
        if (!((($this$isSaturated$iv - 1) | 1) == Long.MAX_VALUE)) {
            long readingBefore = this.reading;
            try {
                m1552plusAssignLRDsOJo(half);
                m1552plusAssignLRDsOJo(Duration.m1468minusLRDsOJo(duration, half));
                return;
    ...
    private final void m1551overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m1480toStringimpl(duration)) + '.');
    }
}
